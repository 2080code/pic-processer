/// <reference path="./pic-processer.d.ts" />
 ...
 * @returns {Promise}
 */
function compress(file:File|Blob, options?:PicProcesserTypes.CompressOptions):Promise<string> {
    const defaultOptions:PicProcesserTypes.CompressOptions = {
        ratio: 0.5,
        mime:'image/jpeg',
        maxWidth: null
    }
    const {ratio, mime, maxWidth} = {...defaultOptions, ...options}
    ...
    return new Promise((resolve, reject) => {
        ...
        img.addEventListener('load', () => {
            const canvas:HTMLCanvasElement = document.createElement('canvas'); // 创建canvas元素
...
export default class PicProcesser implements PicProcesserClass {
  constructor() {
